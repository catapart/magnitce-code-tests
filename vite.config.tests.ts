...
export default defineConfig({
    ...
    build: {
        ...
        rollupOptions: {
            ...
            {
                if(source.includes('code-tests.ts') || source.includes('code-tests.js') || source.includes('code-tests.min.js')) { return true; }
            },
            output: {
    ...
    plugins: [{
        ...
        transform(code, id) {
            const libraryMatch = code.match(/['"].*?code-tests.*?["']/g);
            if(libraryMatch == null) { return null; }
            // console.log(libraryMatch);
            const result = code.replace(libraryMatch[0], '"../libs/code-tests.min.js"');
            // console.log(result);
            return {
                code: result,
